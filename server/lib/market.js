...
const UNIVERSE = {
  ...
  // watchlist — not currently held, candidates for "opportunity" calls
...
function buildSnapshot(dateKey, intradayNonce) {
  const tickers = Object.entries(UNIVERSE).map(([ticker, meta]) => {
    ...
    const changePct = +(((rng() - 0.5) * 6)).toFixed(2); // -3% .. +3% daily move
    ...
    return {
      ...
      price, changePct, volumeM,
      headline, daysToEarnings,
    };
  });

  const vixRng = mulberry32(seedFromString(dateKey + ':VIX' + (intradayNonce || '')));
  const vix = +(14 + vixRng() * 14).toFixed(1); // 14–28 range
...
// Adds a small extra jitter seeded by the current hour, so an intraday
// risk-check called multiple times in the same hour is stable but differs
// hour to hour — simulating "checking the live tape" without a real feed.
function getIntradaySnapshot() {
  const now = new Date();
  const hourKey = dateStr(now) + 'T' + now.getUTCHours();
  return buildSnapshot(dateStr(now), hourKey);
}

module.exports = { getDailySnapshot, getIntradaySnapshot, UNIVERSE };
